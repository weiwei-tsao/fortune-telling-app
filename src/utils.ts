 ...
 */
export function generateTossResult(): TossResult {
 ...
 */
export function generateOriginalHexagramCode(yaos: Yao[]): string {
  if (yaos.length !== 6) {
    throw new Error("Exactly 6 Yao are required to generate a hexagram")
  }

  const code = yaos
    .map((yao) => (yao.type === "少陽" || yao.type === "老陽" ? "1" : "0"))
    .join("")
 ...
 */
export function generateChangedHexagramCode(yaos: Yao[]): string {
  if (yaos.length !== 6) {
    throw new Error("Exactly 6 Yao are required to generate a hexagram")
  }

  const changedYaos = yaos.map((yao) => {
    if (yao.type === "老陽") {
      return { type: "少陰" } // ⚊ → ⚋
    } else if (yao.type === "老陰") {
      return { type: "少陽" } // ⚋ → ⚊
 ...
 */
export function getHexagram(code: string): Hexagram {
